...
def _build_owner_context(history_section: str, cross_context: str = "") -> dict:
    ...
    return {
        "context": (
            ...
            "- nickname: apelido da pessoa (ex: Bebel, Zé, Bel)\n"
            "- pet_name: nome do animal de estimação do contato (ex: Rex, Mel) — só use se André mencionar o pet\n"
            "- Use NULL apenas para LIMPAR explicitamente um campo que André pediu para apagar\n"
            "- Se o campo não foi mencionado, OMITA-O do EXEC (não inclua =NULL)\n"
            "Exemplo: EXEC: update contact Bebel relationship=Filho manual_relationship=Filho nickname=Bebel name=Isabel\n"
