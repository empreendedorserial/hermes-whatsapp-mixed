...
Cohesion: 0.06
Nodes (21): Testes para o fluxo _pending_contact_update: não encontrado → pede número → apli, Quando contato não é encontrado, armazena pendência e pergunta o número., Quando dono responde com número, pendência é aplicada e removida., Testes para _check_chat_silenced., Segunda chamada dentro do TTL não deve fazer HTTP., Testes para comandos do owner em pre_gateway_dispatch., update contact Isabel relationship=Filha' executa e retorna skip., update contact Isabel' sem campos avisa o owner. (+13 more)
...
Cohesion: 0.13
Nodes (14): Testes para _update_full_summary, _compress_full_summary e _sync_full_summaries., _update_full_summary deve retornar texto do LLM., Sem chave de API, _update_full_summary deve retornar None., _compress_full_summary deve retornar 1-2 frases do LLM., _sync_full_summaries deve passar ao LLM apenas mensagens role=user (contato)., Sem state.db, retorna 0 sem erros., _sync_full_summaries não deve processar o número do owner., TestFullSummaryFunctions (+6 more)

### Community 69 - "_call_llm_api"
Cohesion: 0.36
Nodes (4): Testes para _call_llm_api., TestCallLlmApi, _call_llm_api(), Envia uma requisição HTTP POST para uma API de LLM e extrai o texto da resposta.

### Community 71 - "TestBuildLidPhoneMap"
Cohesion: 0.29
Nodes (4): Testa _build_lid_phone_map — construção do mapa LID→telefone., TestBuildLidPhoneMap, _build_lid_phone_map(), Constrói mapa {lid → phone_digits} a partir de três fontes, em ordem de priorida
...
Cohesion: 0.12
Nodes (18): WhatsApp Manager Plugin Package Entry Point., commit_file_to_repo(), _ensure_google_libs(), _load_status_notified(), _load_turn_sent_from_disk(), pre_tool_call(), _pull_and_merge_configurations(), Restaura _turn_sent do disco na inicialização, ignorando entradas expiradas. (+10 more)
...
### Community 86 - "TestRunSyncInBackground"
Cohesion: 0.38
Nodes (4): Testes para _run_sync_in_background., TestRunSyncInBackground, Executa o sync de contatos em thread daemon, notificando o owner ao terminar., _run_sync_in_background()
