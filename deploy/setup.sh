...
$PYTHON_BIN -c "import yaml, os; p='/opt/data/.hermes/profiles/whatsapp/config.yaml'; os.makedirs(os.path.dirname(p), exist_ok=True); ts_list=['file_operations','code_execution','vision','image_generation','video_analysis','video_generation','web_search','browser','terminal','text_to_speech','skills','task_planning','todo','memory','context_engine','session_search','clarifying_questions','clarify','task_delegation','delegate_task','cronjob','cron_jobs','home_assistant','spotify','discord','discord_admin','yuanbao','computer_use','x_search','twitter_search']; ts_cfg={k: False for k in ts_list}; ts_cfg['enabled']=False; cfg={'model': {'default': 'gemini-3.1-flash-lite', 'provider': 'gemini'}, 'agent': {'tool_use_enforcement': 'disabled'}, 'tools': ts_cfg, 'toolsets': ts_cfg, 'skills': {'enabled': False}, 'terminal': {'backend': 'disabled'}, 'display': {'show_system_header': False, 'tool_progress': 'off'}}; open(p, 'w').write(yaml.dump(cfg, default_flow_style=False))" 2>/dev/null || true
